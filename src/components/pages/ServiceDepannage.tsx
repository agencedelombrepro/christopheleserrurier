          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1.4fr 0.6fr', gap: '4rem', alignItems: 'start' }}>
            <div className="reveal">
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', marginTop: '2rem' }}>
                {[
                  { icon: '🔒', title: 'Porte claquée', text: 'Ouverture sans casse si possible. Intervention rapide.' },
                  { icon: '🛡', title: 'Après effraction', text: 'Sécurisation immédiate. Remplacement serrure.' },
                  { icon: '🔑', title: 'Clé perdue', text: 'Ouverture + changement de cylindre sur place.' },
                  { icon: '⚙️', title: 'Serrure bloquée', text: 'Diagnostic et remplacement si nécessaire.' },
                ].map((item) => (
                  <div key={item.title} style={{ background: 'var(--gray-50)', borderRadius: '12px', padding: '1.2rem', border: '1px solid var(--gray-200)' }}>
                    ...
                    <div style={{ fontFamily: "'Syne',sans-serif", fontWeight: 700, fontSize: '.9rem', color: 'var(--navy)', marginBottom: '.3rem' }}>{item.title}</div>
